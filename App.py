...
class MainWindow():
    def __init__(self, window, cap): 
        ...
        self.label = tk.Label(self.window, text="Gesture Detection", font=("Calibri", 12), relief="groove", width=35)
        ...
        self.canvas3.grid(row=4, column=0, padx=10) # to RGB
        self.canvas4.grid(row=4, column=2, padx=10) # to RGB
    ...
    def activate_effects(self):
        ...
        if self.flag_loc:
            self.image_proc, boundrec, contours, contour_output = self.localization(self.image_proc)
            image_copy = self.image.copy()
            self.image_proc_focus = self.resize_image_boundrec(image_copy, boundrec)
            if self.flag_detect:
                image_copy, cnt = self.gesture_detection(image_copy, contours)
                # resize on bounding box
                self.image_proc_focus_gesture = self.resize_image_boundrec(image_copy, boundrec)
                # Write result
                fact = str(cnt) 
    ...
    def skin_ycc_detection(img):
        ...
        ycc_upper = np.array([235, 173, 127],np.uint8) # 235
    ...
    def localization(thres_output): # need input from processed image of a treshold
        ...
        return contour_output, boundrec, contours, contour_output
    ...
    def img_to_photo(img):
        ...
        return photo
    @staticmethod
    def gesture_detection(image_copy, contours):
        #  convexity defect
        contours = max(contours, key=lambda x: cv2.contourArea(x))
        cv2.drawContours(image_copy, [contours], -1, (255,255,0), 2)
        hull = cv2.convexHull(contours)
        cv2.drawContours(image_copy, [hull], -1, (0, 255, 255), 2)
        ...
        for i in range(defects.shape[0]):  # calculate the angle
            ...
            if angle <= np.pi / 2:  # angle less than 90 degree, treat as fingers
                cnt += 1
                cv2.circle(image_copy, far, 4, [0, 0, 255], -1)
        if cnt > 0:
            cnt = cnt+1
        return image_copy, cnt
